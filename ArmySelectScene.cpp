...
void ArmySelectScene::Initialize() {
    ...
    AddNewObject(new Engine::Image("play/sand.png", 1200, 0, 375, 896));
    ...
    AddNewObject(UISpaceUsage = new Engine::Label("Space:0/5", "pirulen.ttf", 25, 1395, 150, 0, 0, 0, 255, 0.8, 0.8));
    ...
    // Add  a large hp army
    ArmyImage[2] = "play/HpArmy.png";
    // Add frozen spell
    ArmyImage[3] = "play/ice.png";
    //Add snow
    ArmyImage[4] = "play/snowflake.png";
    // Add new enemy
    for (int i=0; i<totalArmy; i++) {
    ...
    // TODO 1 (7/8): Create the reset button. You can imitate the enter button construction in the Initialize() function. Done
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER && usedSpace > 0) {
        ...
        if( usedSpace < 5){
            usedSpace += 1;
            armyAmount[id] += 1;
            UISpaceUsage->Text = "Space: "+std::to_string(usedSpace)+"/5";
            UIArmyAmount[id]->Text = std::to_string(armyAmount[id]);
        }
//        UIArmyAmound[id]->Text = ;
...
void ArmySelectScene::AddNewArmy(int id, std::string imageName, int spaceCost) {
    ...
    // TODO 1 (3/8): Create the add(+) button. You can imitate the enter button construction in the Initialize() function. Done
    btn = new Engine::ImageButton("stage-select/AddButton.png", "stage-select/AddButton2.png", halfW/4 + offsetW, oneThirdH + 25 + offsetH, 75, 50);
    ...
    // TODO 1 (4/8): Create the sub(-) button. You can imitate the enter button construction in the Initialize() function. Done
    btn = new Engine::ImageButton("stage-select/SubButton2.png", "stage-select/SubButton.png", halfW/4 + offsetW +100, oneThirdH + 25 + offsetH, 75, 50);
    AddNewControlObject(btn);
