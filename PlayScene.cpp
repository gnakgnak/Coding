...
void PlayScene::Initialize() {
    brokenWall[UP].clear();
    brokenWall[DOWN].clear();
    brokenWall[LEFT].clear();
    brokenWall[RIGHT].clear();
	mapState.clear();
	keyStrokes.clear();
	ticks = 0;
	deathCountDown = -1;
    ...
    MapId = 1;
    remain_second =60;
    playtime = al_create_timer(1);
    al_start_timer(playtime);
    
	// Add groups from bottom to top.
    AddNewObject(TIMER = new Engine::Label("60s","pirulen.ttf", 25,1200, 100, 0, 0, 0, 255, 0.8, 0.8));
...
void PlayScene::Update(float deltaTime) {
    ...
    TIMER->Text = std::to_string(remain_second)+" sec";
    if (remain_second ==0) {
    ...
    for ( auto & it : DefenseGroup-> GetObjects ()){
        ...
        if ( defense->id != 4 )only_trap = false ;
    }
    if ( DefenseGroup-> GetObjects ( ) . empty ( ) || only_trap){
        only_trap = false ;
	...
	if (button & 1) {
		if (!CheckOccupied(x, y)) {
            ...
            if (remainId != -1) {
                ...
                if(remainId == 0)preview = new ArcherArmy(0, 0);
                if(remainId ==1)preview = new BombArmy(0,0);
                if(remainId ==2)preview = new HpArmy(0,0);
                if(remainId ==3)preview = new FrozenSpel(0,0);
                if(remainId ==4)preview = new Snoweffect(0,0);
                preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_CANNON2);break;
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_CANNON2:
                    DefenseGroup->AddNewObject(new LowerCannon(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_TRAP:
...
//        AddNewObject(UIArmyAmount[id] = new Engine::Label("x" + std::to_string(armyAmount[id]), "pirulen.ttf", 20.5, 230 + 120 * id, BlockSize * MapHeight + 110, 0, 0, 0, 255, 0.5, 0.5));
        ...
        btn = new ArmyButton("play/floor.png", "play/dirt.png",
            ...
            , 170 + 120 * (id+5),  BlockSize * MapHeight, 0, id);
        // Reference: Class Member Function Pointer and std::bind.
        btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, id));
        UIGroup->AddNewControlObject(btn);
        // Button Label
        AddNewObject(UIArmyAmount[id] = new Engine::Label("x" + std::to_string(armyAmount[id]), "pirulen.ttf", 20.5, 230 + 120 * (id+5), BlockSize * MapHeight + 110, 0, 0, 0, 255, 0.5, 0.5));
//        AddNewObject(UIArmyAmount[id] = new Engine::Label("x" + std::to_string(armyAmount[id]), "pirulen.ttf", 20.5,+ 120 * (id+5), BlockSize * MapHeight + 110, 0, 0, 0, 255, 0.5, 0.5));
    }
}
...
void PlayScene::UIBtnClicked(int id) {
    ...
    if( id == 2)
        preview = new HpArmy(0,0);
    
    if( id == 3)
        preview = new FrozenSpel(0,0);
    if( id == 4)
        preview = new Snoweffect(0,0);
	if (!preview)
		return;
...
bool PlayScene::CheckOccupied(int x, int y) {
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt == TILE_CANNON2) return true;
